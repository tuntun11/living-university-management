    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            oTable = $('#editor_table').dataTable({
                ...
                "bProcessing": true,
                "bServerSide": true,
                "sAjaxSource": "{{{ URL::action('AdminSpecialEventController@getData') }}}",
            ...
            $('form').submit(function(e){
                ...
                $.ajax({
                    type: "POST",
                    url: (create_new=="true") ? "{{ URL::action('AdminSpecialEventController@postCreate') }}" : "{{ URL::action('AdminSpecialEventController@postEdit') }}",
                    data: data,
        ...
        {
            //set title
            $('#formModalLabel').html('สร้างกิจกรรมพิเศษ');
            //reset form
            $('form')[0].reset();
            //set new value
        ...
        {
            //set title
            $('#formModalLabel').html('แก้ไขกิจกรรมพิเศษ');
            //set new value
            $('input[name=new]').val(false);

            //get Data From Ajax
            $.ajax({
                url: "{{ URL::action('AdminSpecialEventController@getById') }}",
                data :
                {
                    '_token' : $('input[name=_token]').val(),
                    'id' : id
        ...
        function openDelete(id)
        {
            if(confirm('ต้องการยืนยันการลบหรือไม่ ?')==true)
            {
                $.ajax({
                    type: "POST",
                    url: "{{ URL::action('AdminSpecialEventController@postDelete') }}",
                    data :
                    {
                        '_token' : $('input[name=_token]').val(),
                        'id' : id
