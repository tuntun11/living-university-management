                    ...
                    <div class="modal-body">
                        <div role="tabpanel">
                            ...
                            <ul class="nav nav-tabs" role="tablist">
                                <li role="presentation" style="display: none;"><a href="#general" aria-controls="home" role="tab" data-toggle="tab">ข้อมูลทั่วไป</a></li>
                                <li role="presentation" class="active"><a href="#contact" aria-controls="profile" role="tab" data-toggle="tab">ข้อมูลติดต่อ</a></li>
                            </ul>

                            <!-- Tab panes -->
                            <div class="tab-content">
                                <div style="margin: 5px;" role="tabpanel" class="tab-pane fade" id="general">

                                    <div class="form-group">
                                        <label for="inputName">ชื่อสถานที่พัก/โรงแรม *</label>
                                ...
                                <div style="margin: 5px;" role="tabpanel" class="tab-pane fade in active" id="contact">
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            oTable = $('#editor_table').dataTable({
                ...
                "bProcessing": true,
                "bServerSide": true,
                "sAjaxSource": "{{{ URL::action('AdminAccomController@getData') }}}",
            ...
            $('form#form').submit(function(e){
                ...
                $.ajax({
                    type: "POST",
                    url: (create_new=="true") ? "{{ URL::action('AdminAccomController@postCreate') }}" : "{{ URL::action('AdminAccomController@postEdit') }}",
                    data: data,
        ...
        function delDbContract(id)
        {
            if(confirm('ต้องการยืนยันการลบผู้ติดต่อนี้หรือไม่ ?')==true)
            {
                $.ajax({
                    type: "POST",
                    url: "{{ URL::action('AdminAccomController@postContactDelete') }}",
                    data :
                    {
                        '_token' : $('input[name=_token]').val(),
                        'id' : id //contact id
        ...
        function openDelete(id)
        {
            if(confirm('ต้องการยืนยันการลบหรือไม่ ?')==true)
            {
                $.ajax({
                    type: "POST",
                    url: "{{ URL::action('AdminAccomController@postDelete') }}",
                    data :
                    {
                        '_token' : $('input[name=_token]').val(),
                        'id' : id
        ...
        {
            //set title
            $('#formModalLabel').html('สร้างสถานที่พัก/โรงแรม');
            //reset form
            $('form')[0].reset();
            //set new value
        ...
        {
            //set title
            $('#formModalLabel').html('แก้ไขสถานที่พัก/โรงแรม');
            //set new value
            $('input[name=new]').val(false);

            //get Data From Ajax
            $.ajax({
                url: "{{ URL::action('AdminAccomController@getById') }}",
                data :
                {
                    '_token' : $('input[name=_token]').val(),
                    'id' : id
                ...
                $('#comboLocation').val(data.location_id);
            });
