...
@section('extraScripts')
    ...
    {{ HTML::script('select2-4.0.0-beta.3/dist/js/select2.min.js') }}
    {{ HTML::style('select2-4.0.0-beta.3/dist/css/select2.min.css') }}
@stop
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            oTable = $('#editor_table').dataTable({
                ...
                "bProcessing": true,
                "bServerSide": true,
                "sAjaxSource": "{{{ URL::action('AdminExpertController@getData') }}}",
            ...
            $('form').submit(function(e){
                ...
                $.ajax({
                    type: "POST",
                    url: (create_new=="true") ? "{{ URL::action('AdminExpertController@postCreate') }}" : "{{ URL::action('AdminExpertController@postEdit') }}",
                    data: $('form').serialize(),
        ...
        function openEdit(id)
        {
            ...
            //get Data From Ajax
            $.ajax({
                url: "{{ URL::action('AdminExpertController@getById') }}",
                data :
                {
        ...
        function openDelete(id)
        {
            if(confirm('ต้องการยืนยันการลบหรือไม่ ?')==true)
            {
                $.ajax({
                    type: "POST",
                    url: "{{ URL::action('AdminExpertController@postDelete') }}",
                    data :
                    {
                        '_token' : $('input[name=_token]').val(),
                        'id' : id
        ...
        function openDeleteRate(id)
        {
            if(confirm('ต้องการยืนยันการลบราคานี้หรือไม่ ?')==true)
            {
                $.ajax({
                    type: "POST",
                    url: "{{ URL::action('AdminExpertController@postRateDelete') }}",
                    data :
                    {
                        '_token' : $('input[name=_token]').val(),
                        'id' : id
