                            ...
                            <div class="tab-content">
                                <div style="margin: 5px;" role="tabpanel" class="tab-pane fade in active" id="general">
                                    ...
                                    <div class="form-group">
                                        <label for="inputName">ชื่อผู้ให้บริการรถยนต์ *</label>
                                        <input type="text" class="form-control" id="inputName" placeholder="" required>
                                    </div>

                                    <div class="form-group">
                                        <label for="textareaAddress">ที่อยู่</label>
                                    ...
                                    <div class="form-group">
                                        <label>เป็นของมูลนิธิแม่ฟ้าหลวงฯ</label>
                                        <label class="radio-inline">
                                            <input type="radio" name="radioIsMflf" id="radioIsMflf1" value="1"> ใช่
                                        </label>
                                        <label class="radio-inline">
                                            <input type="radio" name="radioIsMflf" id="radioIsMflf2" value="0" checked> ไม่ใช่
                                        </label>
                                    </div>
                                </div>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            oTable = $('#editor_table').dataTable({
                ...
                "bProcessing": true,
                "bServerSide": true,
                "sAjaxSource": "{{{ URL::action('AdminCarFacController@getData') }}}",
            ...
            $('form').submit(function(e){
                ...
                data = {
                    ...
                    'is_mflf' : ($('#radioIsMflf1').is(':checked')) ? 1 : 0,
                    'contacts' : contacts
                };

                $.ajax({
                    type: "POST",
                    url: (create_new=="true") ? "{{ URL::action('AdminCarFacController@postCreate') }}" : "{{ URL::action('AdminCarFacController@postEdit') }}",
                    data: data,
        ...
        function delDbContract(id)
        {
            if(confirm('ต้องการยืนยันการลบผู้ติดต่อนี้หรือไม่ ?')==true)
            {
                $.ajax({
                    type: "POST",
                    url: "{{ URL::action('AdminCarFacController@postContactDelete') }}",
                    data :
                    {
                        '_token' : $('input[name=_token]').val(),
                        'id' : id //contact id
        ...
        function openDelete(id)
        {
            if(confirm('ต้องการยืนยันการลบหรือไม่ ?')==true)
            {
                $.ajax({
                    type: "POST",
                    url: "{{ URL::action('AdminCarFacController@postDelete') }}",
                    data :
                    {
                        '_token' : $('input[name=_token]').val(),
                        'id' : id
        ...
        {
            //set title
            $('#formModalLabel').html('สร้างผู้ให้บริการรถยนต์');
            //reset form
            $('form')[0].reset();
            //set new value
        ...
        {
            //set title
            $('#formModalLabel').html('แก้ไขผู้ให้บริการรถยนต์');
            //set new value
            $('input[name=new]').val(false);

            //get Data From Ajax
            $.ajax({
                url: "{{ URL::action('AdminCarFacController@getById') }}",
                data :
                {
                    '_token' : $('input[name=_token]').val(),
                    'id' : id
                ...
                if (data.is_mflf==1)
                {
                    $('#radioIsMflf1').prop('checked', true);
                }
                else
                {
                    $('#radioIsMflf2').prop('checked', true);
                }
            });
