...
@section('content')
        ...
        <div class="panel-body">
            <form id="search">
                <div class="form-group">
                    <select class="form-control" id="comboLocationFood" name="comboLocationFood" style="width: 100%;">
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <div class="form-group">
                            <label for="comboLocation">สถานที่/ร้านอาหาร</label>
                        ...
                        <div class="form-group">
                            <label for="inputName">ชื่อรายการอาหาร *</label>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" id="inputUnit" value="ชุด" required>
                        </div>
                    </div>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            /*Select2*/
            $('#comboLocationFood').select2({
                placeholder: "เลือกสถานที่",
                allowClear: true
            });
            $('#comboLocationFood').on('change', function(e){
            ...
            oTable = $('#editor_table').DataTable({
                ...
                "order": [[4,'desc']],
                "ajax": {
                    "url" : "{{ URL::action('AdminRestFoodsController@getData') }}",
                    "data" : function(d)
                    {
                        d.location_id = $("#comboLocationFood").val();
                    }
                },
            ...
            $('form').submit(function(e){
                ...
                $.ajax({
                    type: "POST",
                    url: (create_new=="true") ? "{{ URL::action('AdminRestFoodsController@postCreate') }}" : "{{ URL::action('AdminRestFoodsController@postEdit') }}",
                    data: data,
        ...
        function openDelete(id)
        {
            if(confirm('ต้องการยืนยันการลบหรือไม่ ?')==true)
            {
                $.ajax({
                    type: "POST",
                    url: "{{ URL::action('AdminRestFoodsController@postDelete') }}",
                    data :
                    {
                        '_token' : $('input[name=_token]').val(),
                        'id' : id
        ...
        {
            //set title
            $('#formModalLabel').html('สร้างรายการอาหาร');
            //reset form
            $('form')[0].reset();
            //set new value
        ...
        {
            //set title
            $('#formModalLabel').html('แก้ไขรายการอาหาร');
            //set new value
            $('input[name=new]').val(false);

            //get Data From Ajax
            $.ajax({
                url: "{{ URL::action('AdminRestFoodsController@getById') }}",
                data :
                {
                    '_token' : $('input[name=_token]').val(),
                    'id' : id
