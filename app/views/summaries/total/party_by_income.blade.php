...
@section('content')
    ...
    <div class="page-header">
        <h3>
            <i class="fa fa-pie-chart"></i> รายงานสรุปรายรับ
        </h3>
    </div>
    ...
    <script type="text/javascript">
        ...
        $(function () {
			...
			$('#process_button').on('click', function(){
				...
				$.ajax({
                    type: "POST",
                    url: "{{ URL::action('SummaryReportController@postPartyByIncome') }}",
                    ...
                    success: function (data) 
					{
						...
						html += '<table id="table_total" class="table table-condensed" cellspacing="0" width="100%">';
		
						html += '<tr class="strong">';
						html += '<td>เดือน</td>';
						html += '<td align="right">รายรับ</td>';
						html += '<td align="right">จำนวนคณะ</td>';
						...
						$.each(data, function(index,item){
							...
							html += '<td align="right">' + addThousandsSeparator(item.total_income) + '</td>';
							html += '<td align="right">' + addThousandsSeparator(parseInt(item.total_party_qty)) + '</td>';
						...
						html += '<td>รวม</td>';
						html += '<td align="right">' + $.number(all_total_income,2) + '</td>';
						html += '<td align="right">' + addThousandsSeparator(parseInt(all_total_party_qty)) + '</td>';
						html += '<td align="right">' + addThousandsSeparator(parseInt(all_total_people_qty)) + '</td>';
						html += '</tr>';
						html += '<tr>';
						html += '<td>รายรับต่อคนต่อทริป</td>';
						html += '<td align="right">' + $.number((all_total_income)/all_total_people_qty,2) + '</td>';
						html += '<td>บาท</td>';
						html += '<td></td>';
						html += '</tr>';
